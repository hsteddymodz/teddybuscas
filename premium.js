 ...
 * @returns {Boolean}
 */
const checkPremiumUser = (tess, _dir) => {
	let status = false;
	Object.keys(_dir).forEach((i) => {
		if (_dir[i].id === tess) {
			status = true;
...
const expiredCheck = (bot, _dir) => {
	setInterval(() => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./database/premium.json", JSON.stringify(_dir));
			idny ? bot.telegram.sendMessage(idny, `Seu premium acabou, fale com meu dono para adquirir premium novamente.`) : "";
			idny = false;
		}
	}, 1000);
};
